...
class ProductionMeta(BaseModel):
    ...
    provider: Optional[str] = Field(
        default=None, description="所用 Provider 名称"
    )
    asset_url: Optional[str] = Field(
        default=None, description="产物文件地址"
    )
...
class SceneIR(BaseModel):
    ...
    scene_id: str = Field(
        default_factory=lambda: str(uuid4()), description="分镜 ID"
    )
    ...
    visual_spec: VisualSpec = Field(
        default_factory=VisualSpec, description="画面规格"
    )
    ...
    kp_tags: list[str] = Field(
        default_factory=list, description="关联知识点标签"
    )
    source_page: Optional[int] = Field(
        default=None, description="来源页码"
    )
...
class KnowledgePointIR(BaseModel):
    ...
    kp_id: str = Field(
        default_factory=lambda: str(uuid4()), description="知识点 ID"
    )
    title: str = Field(..., description="知识点标题")
    key_points: list[str] = Field(
        default_factory=list, description="要点列表"
    )
    tags: list[str] = Field(
        default_factory=list, description="标签"
    )
    source_ref: str = Field(default="", description="来源引用")
    quiz_seeds: list[QuizSeed] = Field(
        default_factory=list, description="练习题种子"
    )
    scenes: list[SceneIR] = Field(
        default_factory=list, description="分镜列表"
    )
...
class ChapterIR(BaseModel):
    ...
    chapter_id: str = Field(
        default_factory=lambda: str(uuid4()), description="章节 ID"
    )
    title: str = Field(..., description="章节标题")
    order: int = Field(..., description="章节序号")
    source_pages: list[int] = Field(
        default_factory=list, description="来源页码范围"
    )
...
class CourseIR(BaseModel):
    ...
    course_id: str = Field(
        default_factory=lambda: str(uuid4()), description="课程 ID"
    )
